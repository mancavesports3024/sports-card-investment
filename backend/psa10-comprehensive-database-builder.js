...
const PSA10_DATABASE_FILE = path.join(DATABASE_DIR, 'psa10_comprehensive_database.json');
const PSA10_SUMMARY_FILE = path.join(DATABASE_DIR, 'psa10_comprehensive_summary.json');
...
async function collectPSA10Data(targetItemsPerSearch = 30) {
  console.log('🔍 PSA 10 Comprehensive Database Builder');
  console.log('=========================================\n');
  ...
  for (const config of SEARCH_CONFIGS) {
    ...
    try {
      ...
      console.log(`   ✅ Found: ${items.length} items, Added: ${addedCount} new items`);
      successfulSearches++;
...
async function main() {
  try {
    const targetItemsPerSearch = process.argv[2] ? parseInt(process.argv[2]) : 30;
    const database = await collectPSA10Data(targetItemsPerSearch);
