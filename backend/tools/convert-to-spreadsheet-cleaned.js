...
function workbookToCSV(workbook, outputDir) {
    ...
    Object.entries(workbook.sheets).forEach(([sheetName, sheet]) => {
        const filename = `${sheetName.replace(/[^a-zA-Z0-9]/g, '_')}_cleaned.csv`;
...
function main() {
    ...
    try {
        console.log('Reading cleaned comprehensive card database...');
        ...
        console.log(`Found ${data.sets ? data.sets.length : 0} cleaned card sets`);
        ...
        console.log('Creating CSV spreadsheet for cleaned card sets...');
        const csv = jsonToCSV(data);
        if (csv) {
            const csvFile = path.join(outputDir, 'master_card_database_cleaned.csv');
        ...
        if (combinationsCsv) {
            const combinationsCsvFile = path.join(outputDir, 'sport_year_combinations_cleaned.csv');
        ...
        const summary = {
            ...
            cleaningStats: data.metadata.cleaningStats,
        ...
        const summaryFile = path.join(outputDir, 'spreadsheet_summary_cleaned.json');
        fs.writeFileSync(summaryFile, JSON.stringify(summary, null, 2));
        console.log(`Summary file created: ${summaryFile}`);
        
        console.log('\nSpreadsheet creation completed successfully!');
        console.log(`Total cleaned sets processed: ${summary.totalSets}`);
        console.log(`Total sport-year combinations: ${summary.totalCombinations}`);
        console.log(`Sets removed during cleaning: ${summary.cleaningStats.removedCount}`);
