...
function workbookToCSV(workbook, outputDir) {
    ...
    Object.entries(workbook.sheets).forEach(([sheetName, sheet]) => {
        const filename = `${sheetName.replace(/[^a-zA-Z0-9]/g, '_')}_complete.csv`;
...
function main() {
    ...
    try {
        ...
        if (csv) {
            const csvFile = path.join(outputDir, 'master_card_database_complete.csv');
        ...
        if (combinationsCsv) {
            const combinationsCsvFile = path.join(outputDir, 'sport_year_combinations_complete.csv');
        ...
        const summaryFile = path.join(outputDir, 'spreadsheet_summary_complete.json');
