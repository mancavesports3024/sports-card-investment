...
const GOOD_BUYS_FILE = path.join(DATABASE_DIR, 'improved_good_buy_opportunities.json');
const CACHE_FILE = path.join(DATABASE_DIR, 'improved_good_buy_cache.json');
...
class ImprovedGoodBuyFinder {
  ...
  // IMPROVED: Extract card identifier with better strategy
  extractCardIdentifier(psa10Title) {
    // Keep the original title for reference
    ...
    // Strategy 1: Player + RC + Year (most reliable)
    ...
    // Strategy 2: Player + RC + Year (simpler)
    ...
    // Strategy 3: Just player + year
    if (year) {
      const playerMatch2 = psa10Title.match(/([A-Za-z\s]+)\s+\d{4}/i);
      if (playerMatch2) {
  ...
  // IMPROVED: Search card versions with multiple strategies
  async searchCardVersions(cardInfo) {
    ...
    for (const strategy of searchStrategies) {
      ...
      if (results.raw && results.raw.avgPrice > 0) {
        console.log(`✅ Found good results with strategy: ${strategy.name}`);
  ...
  async processCardGroup(group) {
    ...
    if (versions && versions.raw && versions.raw.avgPrice >= MIN_RAW_PRICE) {
      ...
      if (multiplier >= TARGET_MULTIPLIER) {
        ...
        console.log(`   Raw: $${versions.raw.avgPrice.toFixed(2)} | PSA 10: $${psa10Price.toFixed(2)} | Count: ${cards.length}`);
      }
    }
...
async function main() {
  const finder = new ImprovedGoodBuyFinder();
...
module.exports = ImprovedGoodBuyFinder; 