...
function debugStarcadeFilter() {
    console.log('🔍 Debugging Starcade PSA 10 Filter...\n');
    
    const cardTitle = '2024 Panini Select Xavier Worthy 17 STARCADE Silver Prizm, Case Hit SSP PSA 10';
    const card = {
        title: cardTitle,
        price: { value: '185.00' }
    };
    ...
    console.log('🔍 Specific Term Analysis:');
    console.log(`  Contains "silver": ${title.includes('silver')}`);
    console.log(`  Contains "prizm": ${title.includes('prizm')}`);
    console.log(`  Contains "starcade": ${title.includes('starcade')}`);
    console.log(`  Contains "ssp": ${title.includes('ssp')}`);
    console.log(`  Contains "case hit": ${title.includes('case hit')}`);
