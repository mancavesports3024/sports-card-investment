...
class DatabaseDrivenStandardizedTitleGenerator {
    ...
    extractProduct(title) {
        ...
        const productPatterns = [
            ...
            { pattern: 'finest', product: 'Finest' },
            { pattern: 'mosaic', product: 'Mosaic' },
            { pattern: 'select', product: 'Select' },
            { pattern: 'contenders', product: 'Contenders' },
            { pattern: 'hoops', product: 'Hoops' },
            { pattern: 'pokemon', product: 'Pokemon' },
        ...
        // Additional fallback patterns for missing products
        if (titleLower.includes('finest')) {
            return 'Finest';
        }
        if (titleLower.includes('mosaic')) {
            return 'Mosaic';
        }
        if (titleLower.includes('select')) {
            return 'Select';
        }
        if (titleLower.includes('contenders')) {
            return 'Contenders';
        }
        if (titleLower.includes('hoops')) {
            return 'Hoops';
        }
        if (titleLower.includes('pokemon')) {
            return 'Pokemon';
        }
        if (titleLower.includes('o-pee-chee') || titleLower.includes('o pee chee')) {
            return 'O-Pee-Chee';
        }
    ...
    extractPlayer(title) {
        ...
        // Remove specific problematic patterns that are causing ALL CAPS player names
        cleanTitle = cleanTitle.replace(/\bF1\b/g, ''); // Remove F1
        cleanTitle = cleanTitle.replace(/\bHOF\b/g, ''); // Remove HOF
        cleanTitle = cleanTitle.replace(/\bTWINS\b/g, ''); // Remove TWINS
        cleanTitle = cleanTitle.replace(/\bTP\b/g, ''); // Remove TP
        cleanTitle = cleanTitle.replace(/\bHB\b/g, ''); // Remove HB
        cleanTitle = cleanTitle.replace(/\bCJ\b/g, ''); // Remove CJ
        cleanTitle = cleanTitle.replace(/\bBA\b/g, ''); // Remove BA
        cleanTitle = cleanTitle.replace(/\bLK\b/g, ''); // Remove LK
        cleanTitle = cleanTitle.replace(/\bSTARCADE\b/g, ''); // Remove STARCADE
        cleanTitle = cleanTitle.replace(/\bCOLLECTION\b/g, ''); // Remove COLLECTION
        cleanTitle = cleanTitle.replace(/\bMETS\b/g, ''); // Remove METS
        cleanTitle = cleanTitle.replace(/\bBREWERS\b/g, ''); // Remove BREWERS
        cleanTitle = cleanTitle.replace(/\bFEVER\b/g, ''); // Remove FEVER
        cleanTitle = cleanTitle.replace(/\bLONGHORNS\b/g, ''); // Remove LONGHORNS
        cleanTitle = cleanTitle.replace(/\bRUSH\b/g, ''); // Remove RUSH
        cleanTitle = cleanTitle.replace(/\bSPURS\b/g, ''); // Remove SPURS
        cleanTitle = cleanTitle.replace(/\bWJ\b/g, ''); // Remove WJ
        cleanTitle = cleanTitle.replace(/\bPA\b/g, ''); // Remove PA
        cleanTitle = cleanTitle.replace(/\bTMC\b/g, ''); // Remove TMC
        cleanTitle = cleanTitle.replace(/\bHYPER\b/g, ''); // Remove HYPER
        cleanTitle = cleanTitle.replace(/\bLAZER\b/g, ''); // Remove LAZER
        cleanTitle = cleanTitle.replace(/\bKALEIDOSCOPIC\b/g, ''); // Remove KALEIDOSCOPIC
        cleanTitle = cleanTitle.replace(/\bETCH\b/g, ''); // Remove ETCH
        cleanTitle = cleanTitle.replace(/\bRATED\b/g, ''); // Remove RATED
        cleanTitle = cleanTitle.replace(/\bROOKIE\b/g, ''); // Remove ROOKIE
        cleanTitle = cleanTitle.replace(/\bRC\b/g, ''); // Remove RC
        cleanTitle = cleanTitle.replace(/\bSSP\b/g, ''); // Remove SSP
        cleanTitle = cleanTitle.replace(/\bGEM\b/g, ''); // Remove GEM
        cleanTitle = cleanTitle.replace(/\bMINT\b/g, ''); // Remove MINT
        
        // Remove additional problematic patterns from the error examples
        cleanTitle = cleanTitle.replace(/\bS\s+BEST\s+OF\b/g, ''); // Remove "S BEST OF"
        cleanTitle = cleanTitle.replace(/\bKELLOGG'S?\s+D\b/g, ''); // Remove "Kellogg'S D" or "Kellogg'S D"
        cleanTitle = cleanTitle.replace(/\bJERSEY\b/g, ''); // Remove "JERSEY"
        cleanTitle = cleanTitle.replace(/\bV\d+\b/g, ''); // Remove "V341", "V123", etc.
        cleanTitle = cleanTitle.replace(/\bORIGINS\b/g, ''); // Remove "ORIGINS"
        cleanTitle = cleanTitle.replace(/\bO\s+PEE\s+CHEE\b/g, ''); // Remove "O PEE CHEE"
        cleanTitle = cleanTitle.replace(/\bOPC\b/g, ''); // Remove "OPC"
        cleanTitle = cleanTitle.replace(/\bOILERS\b/g, ''); // Remove "OILERS"
        cleanTitle = cleanTitle.replace(/\bKINGS\b/g, ''); // Remove "KINGS"
        cleanTitle = cleanTitle.replace(/\bB\d+\b/g, ''); // Remove "B20", "B123", etc.
        cleanTitle = cleanTitle.replace(/\bJDU\b/g, ''); // Remove "JDU"
        cleanTitle = cleanTitle.replace(/\bDT\d+\b/g, ''); // Remove "DT36", "DT1", etc.
        cleanTitle = cleanTitle.replace(/\bMT\b/g, ''); // Remove MT
        cleanTitle = cleanTitle.replace(/\bGEM MT\b/g, ''); // Remove GEM MT
        cleanTitle = cleanTitle.replace(/\bGEM MINT\b/g, ''); // Remove GEM MINT
        cleanTitle = cleanTitle.replace(/\bPSA\b/g, ''); // Remove PSA
        cleanTitle = cleanTitle.replace(/\b10\b/g, ''); // Remove 10
        cleanTitle = cleanTitle.replace(/\bPSA 10\b/g, ''); // Remove PSA 10
        cleanTitle = cleanTitle.replace(/\bPSA10\b/g, ''); // Remove PSA10
        cleanTitle = cleanTitle.replace(/\bAUTO\b/g, ''); // Remove AUTO
        cleanTitle = cleanTitle.replace(/\bAUTOGRAPH\b/g, ''); // Remove AUTOGRAPH
        cleanTitle = cleanTitle.replace(/\bAUTOGRAPHS\b/g, ''); // Remove AUTOGRAPHS
        cleanTitle = cleanTitle.replace(/\bPROSPECT\b/g, ''); // Remove PROSPECT
        cleanTitle = cleanTitle.replace(/\bPROSPECTS\b/g, ''); // Remove PROSPECTS
        cleanTitle = cleanTitle.replace(/\b1ST\b/g, ''); // Remove 1ST
        cleanTitle = cleanTitle.replace(/\bFIRST\b/g, ''); // Remove FIRST
        cleanTitle = cleanTitle.replace(/\bBOWMAN\b/g, ''); // Remove BOWMAN
        cleanTitle = cleanTitle.replace(/\bCHROME\b/g, ''); // Remove CHROME
        cleanTitle = cleanTitle.replace(/\bDRAFT\b/g, ''); // Remove DRAFT
        cleanTitle = cleanTitle.replace(/\bUNIVERSITY\b/g, ''); // Remove UNIVERSITY
        cleanTitle = cleanTitle.replace(/\bREFRACTOR\b/g, ''); // Remove REFRACTOR
        cleanTitle = cleanTitle.replace(/\bX-FRACTOR\b/g, ''); // Remove X-FRACTOR
        cleanTitle = cleanTitle.replace(/\bALL\b/g, ''); // Remove ALL
        cleanTitle = cleanTitle.replace(/\|/g, ''); // Remove pipe character
        cleanTitle = cleanTitle.replace(/\bCY\b/g, ''); // Remove CY
        cleanTitle = cleanTitle.replace(/\bKAYFUS\b/g, ''); // Remove KAYFUS
    ...
    generateStandardizedTitle(title, playerName = null) {
        ...
        // Ensure player name is included if we have one but it's not in the title
        if (player && !standardizedTitle.includes(player)) {
            // Insert player name after product if we have both
            if (product) {
                const productIndex = standardizedTitle.indexOf(product);
                if (productIndex !== -1) {
                    const afterProduct = standardizedTitle.substring(productIndex + product.length);
                    standardizedTitle = standardizedTitle.substring(0, productIndex + product.length) + ' ' + player + afterProduct;
                } else {
                    standardizedTitle = standardizedTitle + ' ' + player;
                }
            } else {
                standardizedTitle = standardizedTitle + ' ' + player;
            }
