...
class NewPricingDatabase {
    ...
    async extractPlayerName(title) {
        // Extract player name from card title using comprehensive filtering
        // Remove all card-related terms, grading info, team names, and sport terms
        const titleLower = title.toLowerCase();
        ...
        // SIMPLIFIED FILTERING: Single comprehensive approach using the documented terms
        // This replaces all the complex multi-stage filtering with one clean solution
        ...
        // Step 2: Split into words and filter using comprehensive term list
        ...
        for (const word of words) {
            ...
            // COMPREHENSIVE CARD TERMS ARRAY - Consolidated from ALL duplicate files
            // This is the SINGLE source of truth for all card-related filtering terms
            const cardTerms = [
            ...
            // COMPREHENSIVE TEAM NAMES ARRAY - Consolidated from ALL duplicate files
            // This is the SINGLE source of truth for all team name filtering
            const teamNames = [
                // NFL Teams
                ...
                // Individual team names (for partial matches)
            ...
            // COMPREHENSIVE CITY NAMES ARRAY - Filter out city names that shouldn't be in player names
            const cityNames = [
            ...
            if (cityNames.includes(word.toLowerCase())) {
                if (verboseExtraction) console.log(`🔍 Filtered out city name: "${word}"`);
                continue;
            }
            
            // COMPREHENSIVE SPORT TERMS ARRAY - Filter out sport names
            const sportTerms = [
            ...
            if (sportTerms.includes(word.toLowerCase())) {
                if (verboseExtraction) console.log(`🔍 Filtered out sport term: "${word}"`);
                continue;
            }
        ...
        if (filteredWords.length >= 2 && filteredWords.length <= 3) {
            ...
            const bannedWithinName = new Set([
                ...
                // Additional card terms
                'chrome', 'prizm', 'optic', 'mosaic', 'select', 'finest', 'bowman', 'topps', 'panini', 'donruss', 'refractor', 'parallel', 'rookie', 'rc', 'auto', 'autograph', 'base', 'holo', 'ssp', 'sp', 'hof', 'gold', 'silver', 'bronze', 'platinum', 'diamond', 'emerald', 'ruby', 'black', 'white', 'red', 'blue', 'green', 'yellow', 'orange', 'purple', 'pink', 'teal', 'aqua', 'cyan', 'lime', 'mint', 'peach', 'salmon', 'tan', 'brown', 'gray', 'grey', 'navy', 'maroon', 'burgundy', 'crimson', 'scarlet',
                // Card features
                'graded', 'ungraded', 'cert', 'certificate', 'pop', 'population', 'hit', 'case', 'prospect', 'prospects', 'draft', '1st', 'first', 'jersey', 'patch', 'relic', 'memorabilia', 'short print', 'super short print', 'holographic', 'x-fractor', 'cracked ice', 'stained glass', 'die-cut', 'die cut', 'wave', 'velocity', 'scope', 'hyper', 'invicta', 'all-etch', 'edition',
                // Additional patterns
                'flash', 'fifa', 'cosmic', 'planetary', 'pursuit', 'eris', 'autos', 'woo', 'red/white/blue', 'tf1', 'night', 'cosmic stars', 'all etch', 'stars', 'splash', 'rising', 'best', 'genesis', 'fast break', 'zoom', 'flashback', 'emergent', 'mania', 'geometric', 'honeycomb', 'kaleidoscopic', 'vintage', 'downtown', 'real one', 'clear cut', 'variation', 'logo', 'pulsar', 'snakeskin', 'dragon scale', 'tie-dye', 'disco', 'neon', 'camo',
                // Team and city names
