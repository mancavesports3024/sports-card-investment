...
class ComprehensiveCleanup {
    ...
    async initialize() {
        try {
            console.log('🚀 Initializing comprehensive cleanup...');
    ...
    async comprehensiveCleanup() {
        try {
            ...
            // Filter card sets that should be removed
            const setsToRemove = cardSets.filter(set => 
                set.brand === 'Unknown' || 
                set.brand === 'unknown' ||
                set.set_name === 'Unknown' ||
                set.set_name === 'unknown' ||
                set.set_name.includes('Unknown') ||
                set.year === 'Unknown' ||
                set.year === 'unknown'
            );

            console.log(`📊 Found ${setsToRemove.length} card sets to remove`);

            if (setsToRemove.length === 0) {
                console.log('✅ No problematic card sets found. Database is clean!');
                return;
            }
            ...
            for (const cardSet of setsToRemove) {
                try {
                    console.log(`🗑️  Deleting: "${cardSet.set_name}" (Brand: ${cardSet.brand}, Year: ${cardSet.year})`);
            ...
            console.log(`🗑️  Deleted: ${deletedCount} problematic card sets`);
