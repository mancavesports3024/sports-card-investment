...
const GOOD_BUYS_FILE = path.join(DATABASE_DIR, 'precise_good_buy_opportunities.json');
const CACHE_FILE = path.join(DATABASE_DIR, 'precise_good_buy_cache.json');
...
class PreciseGoodBuyFinder {
  ...
  // PRECISE: Extract card identifier that maintains specific card details
  extractCardIdentifier(psa10Title) {
    ...
    // Create search strategies that maintain card specificity
    const searchStrategies = [];
    
    // Strategy 1: Keep specific parallel/parallel details but remove grading terms
    ...
    // Strategy 2: If it's a numbered parallel, try with just the parallel name
    const numberedMatch = psa10Title.match(/(\d+)\/(\d+)/i);
    if (numberedMatch) {
      // Extract parallel name (e.g., "light blue", "silver", "gold")
      ...
      if (parallelMatch) {
        ...
        if (playerMatch && year) {
          ...
          searchStrategies.push({
            name: 'Parallel Specific',
            query: `${player} ${parallel}`,
            priority: 2
          });
        }
      }
    ...
    // Strategy 3: If it's an auto, try with just player + auto
    if (psa10Title.toLowerCase().includes('auto')) {
      const playerMatch = psa10Title.match(/([A-Za-z\s]+)\s+(?:RC|rookie|rookie card)/i);
      if (playerMatch && year) {
        const player = playerMatch[1].trim();
        searchStrategies.push({
  ...
  // PRECISE: Search card versions with validation to ensure matches
      ...
      for (const search of searches) {
        try {
          ...
          if (searchResults && searchResults.length > 0) {
            // VALIDATE: Filter results to ensure they match the specific card type
  ...
  // VALIDATE: Ensure search results match the specific card type
  validateCardMatches(searchResults, originalTitle, searchType) {
    ...
    for (const result of searchResults) {
      ...
      // Check if result has the same parallel type
      if (parallel && !resultLower.includes(parallel)) {
        isValid = false;
      }
      
      // Check if result has the same card number
    ...
    if (versions && versions.raw && versions.raw.avgPrice >= MIN_RAW_PRICE) {
      ...
      if (multiplier >= TARGET_MULTIPLIER) {
        const opportunity = {
          ...
          count: cards.length,
          searchStrategy: cardInfo.searchStrategies[0].name,
...
async function main() {
  const finder = new PreciseGoodBuyFinder();
...
module.exports = PreciseGoodBuyFinder; 